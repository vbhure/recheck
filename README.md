...
Built with the [Strands Agents SDK](https://strandsagents.com) for the AWS *Agents for Humans* hackathon · Track: **Professional Agents** · Demo video: [youtu.be/L7SeR2CWtTA](https://youtu.be/L7SeR2CWtTA) · MIT licensed
...
2. **Works out which disabilities are of an arm or a leg.** The rating schedule's own anatomy (knee, sciatic nerve, humerus, pes planus…) is recognised by a deterministic lexicon. Clinical and eponymous names outside that vocabulary ("cubital tunnel syndrome", "De Quervain's tenosynovitis") go to a model in at most **one** structured call per letter. *(AI only for the gap)* In the demo and tests those answers are replayed from committed fixtures (see `--scripted` below); one live run on a local model is recorded in [docs/evidence](docs/evidence/live-ollama-run.txt).
...
- **It decides when a person is needed — deterministically.** Materiality separates "the letter didn't say which knee" (irrelevant whenever every possible side gives the same rating) from "the letter didn't say which knee, and that decides between 60% and 70%".
...
| Extremity group, other names | **AI** — at most one Strands Agent structured-output call per letter | strict schema: the name echoed back, a group and a confidence only (no side, number or free text). Extra fields, wrong types, a second answer or a repeated key discard the output. Confidence floor 0.75. "none" vetoed for limb or nerve vocabulary and for letters outside Latin-1. `turns=1`, and the Strands Agent's own retry is off. Wall-clock timeout, with the call on its own thread. At most 40 names per call; a name carrying a percentage, label, date or long number is not sent. Any failure leaves the group **unknown** |
| Side (left / right / both) | deterministic, from the letter's words | handedness and linked conditions ("secondary to right knee") are not the rated side (for the link wordings the parser lists; see Limitations); "both" only from wording that names both sides, and two conflicting side words leave the side unknown |
...
- **1,828 tests**: 686 of them the Table I file (every published cell), 1,142 behavioural. Of those, 421 are the red team's regression tests (`tests/test_rt_*.py`), 254 the deep review's and the final verification's (`tests/test_deep_*.py` and `tests/test_rc2_fixes.py`), and 22 the AgentCore entrypoint's (`tests/test_agentcore_app.py`). On the final commit, on Windows with Python 3.12: 1,825 passed, 3 skipped (the two Anthropic-adapter tests and one that needs the `bedrock-agentcore` package), 0 failed. `pytest` runs them in several minutes (165 s to 464 s in the recorded runs on the Windows test machine, depending on load).
- **Regression tests for every code defect fixed**, including those from an 8-lens hostile review, a test-suite review that mutation-tested the code (34 of 35 reintroduced defects caught; the survivor now has a test), an adversarial red team (53 findings, two rounds of fixes, then a final regression pass over where the fixes met), and a deep review of the release candidate in nine areas, with an independent verifier re-attacking the fixes in seven of them, then a final verification of the merged candidate (clean clones behind a network blocker, a cross-area red team, and a claims audit). Its mutation run broke safety checks 106 ways; the suite caught 97 of the 105 that change behaviour, and the 8 survivors now have tests. The defects, fixes, measurements and known residuals are written up in [docs/ENGINEERING.md](docs/ENGINEERING.md).
...
- **Clean clone:** before the deep review, install, the full suite, the demo and every command in this README were run from a fresh clone with an empty home directory and no credentials, on Python 3.12 and 3.10. On the merged release candidate, before the final verification's fixes, the full suite (1,798 passed, 2 skipped) and the demo were run again on Windows with Python 3.12 and 3.10, in installed environments. A few tests skip themselves: two Anthropic-adapter tests without the optional `anthropic` extra, three that need Windows, two that need a case-insensitive filesystem, and two path-length tests when the temporary directory is too deep for them.
...
- **One live model run, on a local model; none on Bedrock.** The model boundary is exercised through the real Strands structured-output path with scripted payloads, including malformed and adversarial ones. One live call was made: `audit fixtures/letters/07_clinical_terms.txt --model ollama` with `qwen2.5:3b` served by Ollama on the build laptop's CPU. The model put both clinical names in the upper-extremity group, and the report gave 80%, POTENTIAL DISCREPANCY, the same result as the committed fixture. The call took 33 s, so `RECHECK_TIMEOUT_S` was raised from the default 30 s to 240 s. The output and how it was run are in [docs/evidence/live-ollama-run.txt](docs/evidence/live-ollama-run.txt). That is one run on one letter: it shows the path works end to end, not that any model is accurate. No accuracy claim is made for any provider.
