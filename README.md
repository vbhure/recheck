...
- **1,806 tests**: 686 of them the Table I file (every published cell), 1,120 behavioural. Of those, 421 are the red team's regression tests (`tests/test_rt_*.py`) and 254 the deep review's and the final verification's (`tests/test_deep_*.py` and `tests/test_rc2_fixes.py`). `pytest` runs them in several minutes (165 s to 464 s in the recorded runs on the Windows test machine, depending on load).
